...
export function convertColorToString(color: Color): string {
  ...
  if (type === ColorType.hsl || type === ColorType.hsla) {
    colorString = `${type.toString()}(${values[0]}, ${values[1]}%, ${values[2]}%`;
  } else {
    colorString = `${type.toString()}(${values[0]}, ${values[1]}, ${values[2]}`;
  }
  if (values.length === 4) {
...
export function decomposeColor(color: string): Color {
  ...
  const type = ColorType[color.substring(0, marker)] as ColorType;
  const values = color.substring(marker + 1, color.length - 1).split(',').map((value) => parseFloat(value));
  return {type, values};
}
...
export function getLuminance(cssColor: string): number {
  ...
  if (color.type === ColorType.rgb || color.type === ColorType.rgba) {
    const rgb = color.values.map((val) => {
      val /= 255; // normalized
      return val <= 0.03928 ? val / 12.92 : Math.pow((val + 0.055) / 1.055, 2.4);
    });
    return Number((0.2126 * rgb[0] + 0.7152 * rgb[1] + 0.0722 * rgb[2]).toFixed(3)); // Truncate at 3 digits
  } else if (color.type === ColorType.hsl || color.type === ColorType.hsla) {
...
export function fade(cssColor: string, value: number): string {
  ...
  if (color.type === ColorType.rgb || color.type === ColorType.hsl) {
    color.type += 1;
  }
...
export function darken(cssColor: string, coefficient: number): string {
  ...
  if (color.type === ColorType.hsl || color.type === ColorType.hsla) {
    color.values[2] *= 1 - coefficient;
  } else if (color.type === ColorType.rgb || color.type === ColorType.rgba) {
    for (let i = 0; i < 3; i++) {
